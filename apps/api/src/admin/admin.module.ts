 ...
 * import ActionModule：task-42 的内容重载需中断玩家动作，写存档必须走 ActionService 的 CAS 范式。
 *
 * 导出 AdminAuditService：task-39/40/41/42 的管理写操作统一复用它落审计，
 * 避免每个模块各写一份"失败不阻塞"的 try/catch。
 */
@Module({
  imports: [AuthModule, PrismaModule, ContentModule, ContentPackStateModule, ActionModule],
  controllers: [
    AdminShopController,
